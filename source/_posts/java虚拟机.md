...
虚引用（phantom reference）如果一个对象为虚引用，你不能通过虚引用来获取对象实例，设置虚引用的唯一目的是在这个对象被垃圾
回收时能收到一个系统的通知。
...
- 回收方法区
很多人认为方法去（HotSpot VM的永久代）是没有垃圾回收的。java虚拟机规范中也说过可以不要求虚拟机对方法区进行垃圾回收，
而且方法区中进行垃圾回收的“性价比”一般比较低。永久代的垃圾回收主要是回收废弃的常量和无用的类。
以常量池中的字面量为例，假如一个字符串“abc”已经进入了常量池，但是系统中没有一个String对象是“abc”，如果这时发生垃圾回收，
而且必要的话，这个“abc”就会被清理出常量池。
判定一个类是否是无用的类，其条件比较苛刻，需要满足3个条件。一是该类的所有实例被回收（堆中找不到该类的实例），二是加载该
类的ClassLoader已经被回收，三是该类对应的java.lang.Class对象没有在任何地方被引用，无法在任何地方通过反射访问该类的方法。
