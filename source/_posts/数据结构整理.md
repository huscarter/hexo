...
#### ConcurrentHashMap（jdk1.7和jdk1.8的实现又大变化）
1. jdk1.7
...
- get新元素。get方法没有加锁，原因是segment的数组使用了volatile修饰，而且HashEntry的的属性（value）也都是volatile，其余（hash、k和next）为final，确保了读取的是最新的数据。
